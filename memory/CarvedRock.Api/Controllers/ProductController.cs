...
public partial class ProductController : ControllerBase
{
    ...
    private static List<string> LargeStrings = new List<string>();

    [HttpGet]
    [AllowAnonymous]
    public async Task<IEnumerable<ProductModel>> Get(CancellationToken cancelToken, string category = "all")
    {
        // Simulate handling a large string object
        string largeString = new string('A', 1024 * 1024 * 10); // 10 MB string
        LargeStrings.Add(largeString);

        // Keep the large strings list to simulate memory leak
        for (int i = LargeStrings.Count - 1; i >= 0; i--)
        {
            if (LargeStrings[i] == null)
            {
                LargeStrings.RemoveAt(i);
            }
        }
